...
from thortils import ithor_scene_type
...
def _test_method(method, target_class, scene="FloorPlan21"):
    valscenes = tt.robothor_scene_names(scene_type="Val")
    scene_type = ithor_scene_type(scene)
    ...
    if target_class not in targets:
        raise ValueError("{} is not a valid target class. Pick one of these: {}".format(target_class, targets))
